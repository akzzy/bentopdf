...
Host Local Edit on your own server using Nginx.
...
Create `/etc/nginx/sites-available/localedit`:
...
```bash
sudo ln -s /etc/nginx/sites-available/localedit /etc/nginx/sites-enabled/
sudo nginx -t
