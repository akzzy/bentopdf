...
Additionally, many CA servers serve certificates over plain HTTP. When your Local Edit instance is hosted over HTTPS, browsers block these HTTP requests (mixed content policy). The CORS proxy resolves both issues by routing requests through an HTTPS endpoint with proper headers.
...
If you're self-hosting Local Edit, you'll need to deploy your own CORS proxy for digital signatures to work with certificates that require chain fetching.
...
3. **Clone Local Edit and update allowed origins**:

   ```bash
   git clone https://github.com/alam00000/localedit.git
   cd localedit/cloudflare
   ```
...
4. **Deploy the proxy**:
   ...
   Note your worker URL (e.g., `https://localedit-cors-proxy.your-subdomain.workers.dev`).

5. **Rebuild Local Edit with the proxy URL**:
