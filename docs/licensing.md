...
- **No license key is required or provided.** Local Edit intentionally does not use license keys to streamline deployment and reduce operational friction for users.
- Upon purchase, users receive a ZIP archive containing the complete Local Edit source code and binaries.
- The source code provided is identical to the code published in our public open-source repository.

**GitHub Repository:** [github.com/alam00000/localedit](https://github.com/alam00000/localedit)

## Important Notice on Third-Party Components

::: info AGPL Components — Pre-configured via CDN
Local Edit **does not bundle** AGPL-licensed processing libraries in its source code. These components are loaded at runtime from CDN URLs that are **pre-configured by default** — all features work out of the box with zero setup.
...
- Local Edit's core code remains under its dual-license (AGPL-3.0 / Commercial)
...
The commercial license covers Local Edit's own code. If you configure and use AGPL components (PyMuPDF, Ghostscript, CoherentPDF), you must still comply with their respective AGPL-3.0 license terms, which may require source code disclosure if you distribute modified versions.
:::
...
### Do I need a license key to use Local Edit?

No. Local Edit does not use license keys. The commercial license grants you legal rights to use the software in proprietary applications.

### Can I use Local Edit in my SaaS product?
...
Components like CoherentPDF are licensed under AGPL v3 and remain under that license. The commercial license covers Local Edit's own code but does not override third-party AGPL obligations.
