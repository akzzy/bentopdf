...
**Local Edit** is a powerful, privacy-first, client-side PDF toolkit that is self hostable and allows you to manipulate, edit, merge, and process PDF files directly in your browser. No server-side processing is required, ensuring your files remain secure and private.

[![Docker Downloads](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fghcr-badge.elias.eu.org%2Fapi%2Falam00000%2Flocaledit%2Flocaledit&query=%24.downloadCount&logo=docker&label=Docker%20Downloads&color=blue)](https://github.com/alam00000/localedit/pkgs/container/localedit) [![Ko-fi](https://img.shields.io/badge/Buy%20me%20a%20Coffee-yellow?logo=kofi&style=flat-square)](https://ko-fi.com/alio01) ![GitHub Stars](https://img.shields.io/github/stars/alam00000/localedit?style=social)
[![Sponsor me on GitHub](https://img.shields.io/badge/Sponsor-%E2%9D%A4-ff69b4)](https://github.com/sponsors/alam00000)

![Local Edit Tools](public/images/localedit-tools.png)
...
Local Edit is **dual-licensed** to fit your needs:
...
Local Edit does **not** bundle AGPL-licensed processing libraries in its source code, but **pre-configures CDN URLs** so all features work out of the box with zero setup:
...
[![Star History Chart](https://api.star-history.com/svg?repos=alam00000/localedit&type=Date)](https://star-history.com/#alam00000/localedit&Date)

---

## 💖 Thank You to Our Sponsors

We're incredibly grateful to all our sponsors and supporters who help keep Local Edit free and open source!
...
- **High Performance**: Built with modern web technologies, Local Edit is fast and efficient, handling even large PDF files with ease.
- **Completely Free**: Local Edit is a free and open-source tool for everyone.

---

## 🛠️ Features / Tools Supported

Local Edit offers a comprehensive suite of tools to handle all your PDF needs.
...
Local Edit is available in multiple languages:
...
Want to help translate Local Edit into your language? Check out our [Translation Guide](TRANSLATION.md)!

---

## 🚀 Getting Started

You can run Local Edit locally for development or personal use.
...
Run Local Edit instantly from GitHub Container Registry (Recommended):

```bash
docker run -p 3000:8080 ghcr.io/alam00000/localedit:latest
```
...
```bash
docker run -p 3000:8080 localeditteam/localedit:latest
```

**Podman (GHCR):**

```bash
podman run -p 3000:8080 ghcr.io/alam00000/localedit:latest
```

**Podman (Docker Hub):**

```bash
podman run -p 3000:8080 docker.io/localeditteam/localedit:latest
```
...
It is very straightforward to host your own instance of Local Edit using a static web page hosting service. Plus, services such as Netlify, Vercel, and GitHub Pages all offer a free tier for getting started. See [Static Hosting](https://github.com/alam00000/localedit/blob/main/STATIC-HOSTING.md) for details.

### 🏠 Self-Hosting Locally

Since Local Edit is fully client-side, all processing happens in the user's browser and no server-side processing is required. This means you can host Local Edit as simple static files on any web server or hosting platform.
...
The easiest way to self-host is to download the pre-built distribution file from our [GitHub releases](https://github.com/alam00000/localedit/releases). Each release includes a `dist-{version}.zip` file that contains all necessary files for self-hosting.

1. Go to [Local Edit Releases](https://github.com/alam00000/localedit/releases)
...
git clone https://github.com/alam00000/localedit.git
cd localedit
...
Local Edit supports different compression modes for optimized builds:

```bash
# Gzip only (smallest Docker image size)
npm run build:gzip
docker build --build-arg COMPRESSION_MODE=g -t localedit:gzip .

# Brotli only (best compression ratio)
npm run build:brotli
docker build --build-arg COMPRESSION_MODE=b -t localedit:brotli .

# No compression (fastest build time)
npm run build:original
docker build --build-arg COMPRESSION_MODE=o -t localedit:original .

# All formats (default, maximum browser compatibility)
npm run build:all
docker build --build-arg COMPRESSION_MODE=all -t localedit:all .
```
...
Local Edit can use jsDelivr CDN to serve large WASM files (LibreOffice, Ghostscript, PyMuPDF) for improved performance and reduced bandwidth costs:
...
```
localedit-airgap-bundle/
  localedit.tar              # Docker image
...
| `--image-name <name>`          | Docker image tag                                 | `localedit`                       |
| `--output-dir <path>`          | Output bundle directory                          | `./localedit-airgap-bundle`       |
...
> [!IMPORTANT]
> WASM files must be served from the **same origin** as the Local Edit app. Web Workers use `importScripts()` which cannot load scripts cross-origin. For example, if Local Edit runs at `https://internal.example.com`, the WASM base URL should also be `https://internal.example.com/wasm`.
...
```bash
docker save localedit -o localedit.tar
```
...
- `localedit.tar` — the Docker image
- `localedit-pymupdf-wasm-0.11.14.tgz` — PyMuPDF WASM package
- `localedit-gs-wasm-*.tgz` — Ghostscript WASM package
...
docker load -i localedit.tar

# Extract the WASM packages
mkdir -p ./wasm/pymupdf ./wasm/gs ./wasm/cpdf ./wasm/ocr/core ./wasm/ocr/lang-data ./wasm/ocr/fonts
tar xzf localedit-pymupdf-wasm-0.11.14.tgz -C ./wasm/pymupdf --strip-components=1
tar xzf localedit-gs-wasm-*.tgz -C ./wasm/gs --strip-components=1
...
# Run Local Edit
docker run -d -p 3000:8080 --restart unless-stopped localedit
```
...
Local Edit can also be hosted from a subdirectory (e.g., `example.com/tools/localedit/`):

```bash

# Example:
# 1. Build the app with the specific BASE_URL. BASE_URL must have a trailing and leading slash. The BASE_URL can be any url of your choice. Here we are using /tools/localedit/ as an example.

BASE_URL=/tools/localedit/ npm run build
...
mkdir -p serve-test/tools/localedit

# 3. Copy all files from the 'dist' folder into that nested directory
cp -r dist/* serve-test/tools/localedit/

# 4. Serve the 'serve-test' folder
npx serve serve-test
```

The website can be accessible at: `http://localhost:3000/tools/localedit/`
...
Local Edit's Docker image also supports the `BASE_URL` build argument for subdirectory deployments:

```bash
# Build for subdirectory deployment
docker build --build-arg BASE_URL=/localedit/ -t localedit .

# Run the container
docker run -p 3000:8080 localedit

# The app will be accessible at http://localhost:3000/localedit/
```
...
```bash
docker build --build-arg VITE_DEFAULT_LANGUAGE=fr -t localedit .
```
...
docker run -p 3000:8080 localedit-simple
```

> [!IMPORTANT]
>
> - Always include trailing slashes in `BASE_URL` (e.g., `/localedit/` not `/localedit`)
...
services:
  localedit:
    image: ghcr.io/alam00000/localedit:latest # Recommended
    # image: localeditteam/localedit:latest     # Alternative: Docker Hub
    container_name: localedit
    ports:
      - '3000:8080'
...
For Linux production deployments, you can run Local Edit as a systemd service using [Podman Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html).

Create `~/.config/containers/systemd/localedit.container`:

```ini
[Unit]
Description=Local Edit - Privacy-first PDF toolkit
After=network-online.target

[Container]
Image=ghcr.io/alam00000/localedit:latest
ContainerName=localedit
...
For organizations that want a clean, distraction-free interface focused solely on PDF tools, Local Edit supports a **Simple Mode** that hides all branding and marketing content.
...
Replace the default Local Edit logo, name, and footer text with your own. Branding is configured via environment variables at **build time** and works across all deployment methods (Docker, static hosting, air-gapped VMs).

| Variable           | Description                             | Default                                   |
| ------------------ | --------------------------------------- | ----------------------------------------- |
| `VITE_BRAND_NAME`  | Brand name shown in header and footer   | `Local Edit`                              |
...
```bash
docker build --build-arg DISABLE_TOOLS="edit-pdf,sign-pdf,encrypt-pdf" -t localedit .
```
...
Local Edit runs as a non-root user using nginx-unprivileged for enhanced security:
...
```bash
docker build -t localedit .
docker run -p 8080:8080 localedit
```
...
docker build -f Dockerfile.nonroot -t localedit-nonroot .

# Run with custom UID/GID
docker run -d -p 3000:8080 -e PUID=1000 -e PGID=1000 localedit-nonroot
```
...
5. **Note your worker URL** (e.g., `https://localedit-cors-proxy.your-subdomain.workers.dev`)
...
Local Edit uses client-side HMAC as a deterrent against casual abuse, but accepts this tradeoff due to its fully client-side architecture. To enable:
...
Local Edit supports semantic versioning with multiple container tags available:

**GitHub Container Registry (Recommended):**

- **Latest**: `ghcr.io/alam00000/localedit:latest`
- **Specific Version**: `ghcr.io/alam00000/localedit:1.0.0`
- **Version with Prefix**: `ghcr.io/alam00000/localedit:v1.0.0`

**Docker Hub:**

- **Latest**: `localeditteam/localedit:latest`
- **Specific Version**: `localeditteam/localedit:1.0.0`
- **Version with Prefix**: `localeditteam/localedit:v1.0.0`
...
Local Edit was originally built using **HTML**, **CSS**, and **vanilla JavaScript**. As the project grew, it was migrated to a modern stack for better maintainability and scalability:
...
Have an idea for a new tool or an improvement? [Open an issue](https://github.com/alam00000/localedit/issues) to discuss it first.
...
Local Edit wouldn't be possible without the amazing open-source tools and libraries that power it. We'd like to extend our heartfelt thanks to the creators and maintainers of:
...
> [!NOTE]
> AGPL-licensed libraries are not bundled in Local Edit's source code. They are loaded at runtime from CDN (pre-configured) and can be overridden via environment variables or Advanced Settings.
