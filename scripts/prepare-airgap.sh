...
# ============================================================
# Local Edit Air-Gapped Deployment Preparation Script
# ============================================================
...
# contains everything needed to deploy Local Edit offline.
#
# Usage:
...
# --- Defaults ---
WASM_BASE_URL=""
IMAGE_NAME="localedit"
OUTPUT_DIR="./localedit-airgap-bundle"
SIMPLE_MODE=""
BASE_URL=""
COMPRESSION_MODE=""
LANGUAGE=""
...
usage() {
  cat <<'EOF'
Local Edit Air-Gapped Deployment Preparation
...
OPTIONS:
  --image-name <name>     Docker image name          (default: localedit)
  --output-dir <path>     Output bundle directory     (default: ./localedit-airgap-bundle)
...
if [ ! -f "package.json" ] || [ ! -f "src/js/const/cdn-version.ts" ]; then
  error "This script must be run from the Local Edit project root."
...
interactive_mode() {
  echo ""
  echo -e "${BOLD}============================================================${NC}"
  echo -e "${BOLD}  Local Edit Air-Gapped Deployment Preparation${NC}"
  ...
  echo "      https://cdn.mycompany.local/localedit"
  echo ""
  while true; do
  ...
  echo "    Replace the default Local Edit name, logo, and footer text."
  read -r -p "    Brand name [Local Edit]: " input
  BRAND_NAME="${input:-}"
  if [ -n "$BRAND_NAME" ]; then
    ...
    BRAND_LOGO="${input:-}"
    read -r -p "    Footer text [© 2026 Local Edit. All rights reserved.]: " input
    FOOTER_TEXT="${input:-}"
  fi
  echo ""
  ...
  echo "  Brand Name:     ${BRAND_NAME:-Local Edit (default)}"
...
echo ""
echo -e "${BOLD}============================================================${NC}"
echo -e "${BOLD}  Local Edit Air-Gapped Bundle Preparation${NC}"
...
if ls "$OUTPUT_DIR"/*.tgz "$OUTPUT_DIR"/localedit.tar "$OUTPUT_DIR"/setup.sh 2>/dev/null | head -1 &>/dev/null; then
...
if [ "$SKIP_WASM" = true ]; then
  ...
  wasm_missing=false
  if ! ls "$OUTPUT_DIR"/localedit-pymupdf-wasm-*.tgz &>/dev/null; then
    error "Missing: localedit-pymupdf-wasm-*.tgz"
    wasm_missing=true
  fi
  if ! ls "$OUTPUT_DIR"/localedit-gs-wasm-*.tgz &>/dev/null; then
    error "Missing: localedit-gs-wasm-*.tgz"
    wasm_missing=true
  fi
  ...
  info "  PyMuPDF:      $(filesize "$OUTPUT_DIR"/localedit-pymupdf-wasm-*.tgz)"
  info "  Ghostscript:  $(filesize "$OUTPUT_DIR"/localedit-gs-wasm-*.tgz)"
...
if [ "$SKIP_DOCKER" = true ]; then
  ...
  if [ -f "$OUTPUT_DIR/localedit.tar" ]; then
    success "Reusing existing localedit.tar"
  elif docker image inspect "$IMAGE_NAME" &>/dev/null; then
    step "Exporting existing Docker image"
    docker save "$IMAGE_NAME" -o "$OUTPUT_DIR/localedit.tar"
    success "Exported: $(filesize "$OUTPUT_DIR/localedit.tar")"
  else
    warn "No Docker image '${IMAGE_NAME}' found and no localedit.tar in output."
...
# ============================================================
# Local Edit Air-Gapped Setup Script
# Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
# Local Edit v${APP_VERSION}
# ============================================================
...
echo ""
echo "============================================================"
echo "  Local Edit Air-Gapped Setup"
...
if [ -f "\${SCRIPT_DIR}/localedit.tar" ]; then
  docker load -i "\${SCRIPT_DIR}/localedit.tar"
  echo "  Docker image '\${IMAGE_NAME}' loaded."
else
  echo "  WARNING: localedit.tar not found. Skipping Docker load."
...
tar xzf "\${SCRIPT_DIR}"/localedit-pymupdf-wasm-*.tgz -C "\${WASM_DIR}/pymupdf" --strip-components=1

# Ghostscript: browser expects gs.js and gs.wasm at root
echo "  Extracting Ghostscript..."
TEMP_GS="\$(mktemp -d)"
tar xzf "\${SCRIPT_DIR}"/localedit-gs-wasm-*.tgz -C "\${TEMP_GS}"
...
# --- Step 3: Start Local Edit ---
echo ""
echo "[3/3] Ready to start Local Edit"
echo ""
echo "  To start manually:"
echo "    docker run -d --name localedit -p \${DOCKER_PORT}:8080 --restart unless-stopped \${IMAGE_NAME}"
echo ""
echo "  Then open: http://localhost:\${DOCKER_PORT}"
echo ""

read -r -p "Start Local Edit now? (y/N): " REPLY
if [[ "\${REPLY:-}" =~ ^[Yy]$ ]]; then
  docker run -d --name localedit -p "\${DOCKER_PORT}:8080" --restart unless-stopped "\${IMAGE_NAME}"
  echo ""
  echo "  Local Edit is running at http://localhost:\${DOCKER_PORT}"
fi
SETUP_EOF
...
# Local Edit Air-Gapped Deployment Bundle

**Local Edit v${APP_VERSION}** | Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
...
| \`localedit-pymupdf-wasm-${PYMUPDF_VERSION}.tgz\` | PyMuPDF WASM module |
| \`localedit-gs-wasm-${GS_VERSION}.tgz\` | Ghostscript WASM module |
...
3. Optionally start the Local Edit container
...
# PyMuPDF
tar xzf localedit-pymupdf-wasm-${PYMUPDF_VERSION}.tgz -C ./wasm/pymupdf --strip-components=1

# Ghostscript (extract assets/ to root)
TEMP_GS=\$(mktemp -d)
tar xzf localedit-gs-wasm-${GS_VERSION}.tgz -C \$TEMP_GS
...
\`\`\`bash
docker run -d --name localedit -p 3000:8080 --restart unless-stopped ${IMAGE_NAME}
\`\`\`
