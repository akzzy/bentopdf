# Local Edit Helm Chart

Deploys **Local Edit** as a **single NGINX container** serving the static frontend.
...
```bash
helm install localedit ./chart
kubectl port-forward deploy/localedit 8080:8080
...
- **`image.repository`**: container image repo (default: `ghcr.io/alam00000/localedit-simple`)
- **`image.tag`**: image tag (default: `Chart.appVersion`)
- **`image.pullPolicy`**: default `IfNotPresent`

### Ports

- **`containerPort`**: container listen port (**8080** for the Local Edit nginx image)
...
# produces localedit-<version>.tgz
helm push localedit-*.tgz oci://ghcr.io/$GHCR_USERNAME/charts
```
...
```bash
helm upgrade --install localedit oci://ghcr.io/$GHCR_USERNAME/charts/localedit --version 1.0.0
```
